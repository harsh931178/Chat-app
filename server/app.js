...
app.post('/api/register',async(req,res,next)=>{
    try{
        ...
        if(!fullName ||!email||!password){
            res.send(400).send("Please fill all details");
        }
        else{
...
app.get('/api/conversation/:userId',async(req,res)=>{
    try {
        ...
        const conversationUserData=Promise.all(conversations.map(async(conversation)=>{
            ...
            return {user:{recieverId:user._id,email:user.email,fullName:user.fullName},conversationId:conversation._id};
...
app.post('/api/message',async(req,res)=>{
    try {
        ...
        if(!senderId || !message) return res.status(400).send("please fill all details");
